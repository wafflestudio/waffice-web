...
import { ExpenseClaimCreate } from '@/types';
...
export default function ExpenseClaimsPage() {
  ...
  const createExpenseClaimMutation = useMutation({
    mutationFn: (data: ExpenseClaimCreate) => 
      apiClient.createExpenseClaim(data, 1), // Using member ID 1 for demo
    onSuccess: () => {
  ...
  const approveExpenseClaimMutation = useMutation({
    mutationFn: (id: number) => 
      apiClient.approveExpenseClaim(id, 1, 'Approved by admin'), // Using admin ID 1 for demo
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['expense-claims'] });
    },
  });
  ...
  const rejectExpenseClaimMutation = useMutation({
    mutationFn: (id: number) => 
      apiClient.rejectExpenseClaim(id, 1, 'Rejected by admin'), // Using admin ID 1 for demo
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['expense-claims'] });
    },
  });
