...
import type { AccountBalance, Transaction, TransactionCreate } from "@/types"
...
export default function AccountingPage() {
	...
	const _handleCreateTransaction = async (data: TransactionCreate) => {
		...
		setBalance((prev) => ({
			total_income:
				data.transaction_type === "income" ? prev.total_income + data.amount : prev.total_income,
			total_expense:
				data.transaction_type === "expense" ? prev.total_expense + data.amount : prev.total_expense,
				...
				setBalance((prev) => ({
					total_income:
						transaction.transaction_type === "income"
							? prev.total_income - transaction.amount
							: prev.total_income,
					total_expense:
						transaction.transaction_type === "expense"
							? prev.total_expense - transaction.amount
							: prev.total_expense,
					balance: prev.balance + amount,
					transaction_count: prev.transaction_count - 1,
				}))
	...
	const handleAddComment = async (id: number, comment: string) => {
		...
		setTransactions((prev) =>
			prev.map((t) =>
				t.id === id ? { ...t, comments: comment, updated_at: new Date().toISOString() } : t,
			),
		)
	}
